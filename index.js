...
function checkUserType(options) {
   return new Promise( (resolve, reject) => {
      if (!options.user.attributes.user_type) {
         new Parse.Query('User')
         .equalTo('email', options.user.attributes.email)
         .first({useMasterKey: true})
         .then( resp => resolve( resp.get('user_type') ))
         .catch(err => reject(err));
      } else {
         resolve( options.user.attributes.user_type );
      }
   });
}
...
var MandrillAdapter = mandrillOptions => {
  ...
  var sendVerificationEmail = options => {
   ...
   return new Promise((resolve, reject) => {
      ...
      checkUserType(options)
         .then( final_user_type => {

            // Overrides for user types
            var user_type = final_user_type || options.user.get("user_type");
